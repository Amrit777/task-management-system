...
const TaskForm = ({ isEditing = false }) => {
    const [form, setForm] = useState({
        ...
        estimatedEndDate: '',
        actualEndDate: '',
        assignedTo: '', // will be '' or a user ID
        // attachments will be handled as file inputs
    });
    ...
    return (
        ...
        <h2 className="text-2xl mb-4" > { isEditing? 'Edit Task': 'Create New Task' } </h2>
                ...
                <label className="block mb-1" > Start Date </label>
                    < input type = "date" name = "startDate" value = { form.startDate } onChange = { handleChange } className = "w-full p-2 border rounded" />
                        ...
                        <label className="block mb-1" > Estimated Time(hours) </label>
                            < input type = "number" name = "estimatedTime" value = { form.estimatedTime } onChange = { handleChange } className = "w-full p-2 border rounded" />
                                </div>
                                < div >
                                <label className="block mb-1" > Estimated End Date </label>
                                    < input type = "date" name = "estimatedEndDate" value = { form.estimatedEndDate } onChange = { handleChange } className = "w-full p-2 border rounded" />
                                        </div>
                                        < div >
                                        <label className="block mb-1" > Actual Task Ended On </label>
                                            < input type = "date" name = "actualEndDate" value = { form.actualEndDate } onChange = { handleChange } className = "w-full p-2 border rounded" />
                                                </div>
                                                < div >
                                                <label className="block mb-1" > Assigned To: </label>
                                                    < select name = "assignedTo" value = { form.assignedTo } onChange = { handleChange } className = "w-full p-2 border rounded" >
